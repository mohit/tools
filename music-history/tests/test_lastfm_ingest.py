...
def test_infer_resume_from_raw_legacy_runs_for_same_range_uses_conservative_missing_page(tmp_path: Path) -> None:
    raw_root = tmp_path / "lastfm"

    # Older legacy run looks complete for early pages.
    _write_jsonl(raw_root / "scrobbles_run-5201_from-1000_p0001.jsonl", [{"uts": 1001}])
    _write_jsonl(raw_root / "scrobbles_run-5201_from-1000_p0002.jsonl", [{"uts": 1002}])
    _write_jsonl(raw_root / "scrobbles_run-5201_from-1000_p0003.jsonl", [{"uts": 1003}])

    # Newer interrupted legacy run wrote page 1 but checkpoint was never saved.
    _write_jsonl(raw_root / "scrobbles_run-5202_from-1000_p0001.jsonl", [{"uts": 1001}])
...
def test_resolve_start_checkpoint_uses_conservative_missing_page_with_legacy_runs(tmp_path: Path) -> None:
    raw_root = tmp_path / "lastfm"
    _write_jsonl(raw_root / "scrobbles_run-8501_from-1000_p0001.jsonl", [{"uts": 1001}])
    _write_jsonl(raw_root / "scrobbles_run-8501_from-1000_p0002.jsonl", [{"uts": 1002}])
    _write_jsonl(raw_root / "scrobbles_run-8501_from-1000_p0003.jsonl", [{"uts": 1003}])
    _write_jsonl(raw_root / "scrobbles_run-8502_from-1000_p0001.jsonl", [{"uts": 1001}])
